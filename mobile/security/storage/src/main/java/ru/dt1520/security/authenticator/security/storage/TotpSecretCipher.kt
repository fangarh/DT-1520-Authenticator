...
internal interface TotpSecretCipher {
    fun encrypt(plainText: String): SecureTotpSecretRecord

    fun decrypt(record: SecureTotpSecretRecord): String
}

internal class AndroidKeystoreTotpSecretCipher(
    private val keyAlias: String = SecureTotpSecretRecord.DEFAULT_KEY_ALIAS
) : TotpSecretCipher {
    ...
    private fun <T> runCipherOperation(
        ...
        try {
            ...
            throw SecureTotpSecretStorageException(
                message = "Unable to $operation TOTP secret payload.",
                cause = exception
            )
