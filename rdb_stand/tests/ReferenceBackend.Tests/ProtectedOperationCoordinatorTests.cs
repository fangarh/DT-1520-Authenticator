...
public sealed class ProtectedOperationCoordinatorTests
{
    ...
    public async Task StartAsyncCreatesDt1520ChallengeAndReturnsPollingSession()
    {
        ...
        Assert.Equal(ChallengeId, gateway.LastCreatedChallengeId);
    ...
    [Fact]
    public async Task VerifyTotpAsyncApprovesThroughGatewayWithoutPersistingCode()
    {
        var gateway = new FakeAuthenticatorGateway();
        var coordinator = CreateCoordinator(gateway);
        var started = await StartAsync(coordinator);
        ...
        Assert.True(result.IsSuccess);
        Assert.Equal(ChallengeId, gateway.LastVerifyChallengeId);
    ...
    private sealed class FakeAuthenticatorGateway : IReferenceAuthenticatorGateway
    {
        ...
        public Guid? LastCreatedChallengeId { get; private set; }
            ...
            CancellationToken cancellationToken)
        {
            ...
            return Task.FromResult(Dt1520AuthenticatorResult<ChallengeResponse>.Success(CreateChallenge("pending")));
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            LastVerifyCode = code;
            return Task.FromResult(Dt1520AuthenticatorResult<ChallengeResponse>.Success(CreateChallenge("approved")));
        }

        private static ChallengeResponse CreateChallenge(string status)
        {
