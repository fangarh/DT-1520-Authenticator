...
- The Admin UI and Android sections are the current handoff surfaces for installation, permissions, tenant quick create, selected-tenant management, report snapshots, integration client onboarding, secret rotation, deactivation/reactivation, QR device onboarding and troubleshooting.
...
- Callback URL policy is now documented as explicit runtime configuration: `PublicInternet` strict by default, `PrivateNetwork` for closed HTTPS contours and `LocalDevelopment` for local/demo HTTP callbacks only.
- The reference stand handoff points to `rdb_stand/ReferenceDesktopBackendStand.slnx`: ASP.NET Core backend, console desktop shell, WPF MVVM demo shell with encrypted local MVP settings, sanitized `--preflight`/live-readiness checks, env-var runbook and tests for signed callbacks, callback URL policy, status polling and online TOTP fallback.
