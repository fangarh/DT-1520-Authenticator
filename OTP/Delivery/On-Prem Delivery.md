...
Для первого внешнего pilot rollout на личном сервере `ghostring` допускается более узкий server-specific профиль:

- existing host `nginx` как public HTTPS edge
- `api + worker + admin`
- existing `PostgreSQL` / `dt-auth`
- отдельный `Redis`
- без compose-managed `postgres`

Этот профиль зафиксирован отдельно в [[Ghostring Pilot Deployment Profile]] и не отменяет базовый on-prem contract для более общего rollout.

Для него дополнительно подготовлены server-specific assets в репозитории:
