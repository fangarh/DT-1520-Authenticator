...
На `2026-04-15` в репозитории уже есть база для installer-контура:
...
При этом в репозитории теперь уже есть первый packaging slice:
...
При этом пока еще нет:

- отдельного installer binary или `Bootstrap Agent`

Это значит, что design contract уже начал переходить в код, но `Installer MVP` еще не завершен end-to-end.
...
## Следующий практический шаг после packaging slice

Следующая конкретная реализация должна идти в таком порядке:

1. реализовать installer entry point поверх существующих migration/bootstrap команд
2. усилить readiness/reporting и recovery path, особенно для `worker`
3. расширить recovery troubleshooting beyond baseline runbook
4. решить, нужен ли отдельный ingress/API exposure contract сверх текущего `admin` HTTPS edge
