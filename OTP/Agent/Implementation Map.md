...
- `OTP/Delivery/Reference Desktop Backend Stand.md` — будущий минимальный стенд для SDK validation, latency hardening и online `TOTP` fallback
...
- `rdb_stand/` - future reference `Desktop + Backend` stand root; фактическое имя с underscore
- `config/mcp/` - локальные примеры MCP-конфигов
...
- `admin/src/features/device-onboarding/DeviceOnboardingPanels.test.tsx` - component tests для QR device onboarding create/list/detail/revoke panels, `QRCodeSVG` one-time payload display и discard behavior
...
- `admin/e2e/admin-device-onboarding.spec.ts` - checked-in browser regression для QR device onboarding workspace: `login -> load artifacts -> create QR -> one-time payload/QR visible -> discard -> revoke`, browser storage non-persistence и horizontal overflow check
...
- `mobile/feature/device-onboarding/src/test/java/ru/dt1520/security/authenticator/feature/deviceonboarding/DeviceOnboardingPayloadTest.kt` - unit tests для QR activation payload validation contract
- `mobile/feature/device-onboarding/src/test/java/ru/dt1520/security/authenticator/feature/deviceonboarding/DeviceOnboardingWorkflowTest.kt` - unit tests для QR payload import workflow, sanitized invalid-payload copy и successful activation cleanup
