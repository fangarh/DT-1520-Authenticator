...
- `OTP/Delivery/Final Integrated Verification Closure Plan.md` — continuation plan for explicit `TOTP` fallback, combined QR onboarding and Android no-restart pending refresh after the first server-owned live proof
- `OTP/Delivery/Final Integrated Verification Context Reset Prompt.md` — стартовый prompt для продолжения Final Integrated Verification после очистки контекста
...
- `mobile/app/src/test/java/ru/dt1520/security/authenticator/app/DebugDevicePushTokenTest.kt` - unit tests для stable debug-only non-secret push token derivation and blank-input fail-closed behavior
...
- `mobile/app/src/main/java/ru/dt1520/security/authenticator/app/AndroidDevicePushToken.kt` - debug-only development push token resolver: derives stable non-secret token from installation id for pre-FCM live reference proof; release builds return no synthetic token
...
- `2026-04-28`: для ghostring runtime alignment добавлен backend contract test `backend/OtpAuth.Infrastructure.Tests/Integrations/IssueIntegrationTokenResponseTests.cs`, фиксирующий OAuth snake_case shape `/oauth2/token`; `rdb_stand/src/ReferenceBackend/appsettings.Development.example.json` и `rdb_stand/README.md` теперь показывают least-privilege scope `challenges:read challenges:write`
...
- `2026-04-29`: карта дополнена Final Integrated Verification Gate push-capable fix: `AndroidDevicePushToken.kt` добавляет debug-only development token из installation id, `AuthenticatorApp` передает его при QR activation, `DeviceRuntimeSessionManagerTest` фиксирует forwarding `pushToken`, а `DebugDevicePushTokenTest` покрывает stable token/blank handling. Verification: `:app:testDebugUnitTest :app:assembleDebug` passed outside sandbox, fresh APK installed/launched via Android MCP on `emulator-5554`.
- `2026-04-29`: добавлен planning handoff `OTP/Delivery/Final Integrated Verification Closure Plan.md` и context reset prompt. План фиксирует итерации: explicit `TOTP` fallback challenge в `ReferenceBackend`, Android foreground pending refresh без app restart, combined operator QR для device + TOTP enrollment, Android combined consume flow и final live gate.
...
- `2026-04-28`: вместо third-party callback tunnel добавлен server-owned `ghostring` contour для reference stand: `infra/docker/reference-backend.Dockerfile` собирает `rdb_stand/src/ReferenceBackend`, `infra/docker-compose.ghostring.yml` добавляет service `reference-backend` with internal `Dt1520Authenticator__BaseUrl=http://api:8080/` and loopback host publish `127.0.0.1:${REFERENCE_BACKEND_HOST_HTTP_PORT:-15188}:5188`, `infra/nginx/reference-backend.ghostring.ru.conf.example` публикует `https://admin.ghostring.ru:18444/`, а `infra/env/ghostring.runtime.env.example` фиксирует `REFERENCE_BACKEND_*` runtime env contract. `infra/tests/packaging.contract.tests.ps1` покрывает новые assets.
- `2026-04-29`: добавлены `ADR-036` и `Delivery/Optional Boxed Integration Gateway`: optional boxed gateway принят как productization boundary для reference backend, а `RDP + TOTP` выделен в будущий access connector track. Будущие изменения в `rdb_stand`, `lib/`, challenge/session contracts, identity mapping, online `TOTP`, audit and policy должны сохранять reusable services без desktop-polling assumptions.
- `2026-04-29`: `lib/src/Dt1520.Authenticator.Client/Dt1520AuthenticatorHttpPipeline.cs` исправлен после live Linux/container blocker in server-owned ReferenceBackend: rooted API paths such as `/api/v1/challenges` are now combined with configured `BaseUrl` before base-boundary enforcement, while absolute request targets remain limited to `http/https` and still must stay under the configured DT-1520 base URL. Regression tests added in `lib/tests/Dt1520.Authenticator.Client.Tests/ClientHttpFoundationTests.cs`; verification green: `lib` tests `88/88`, `rdb_stand` build, `rdb_stand` tests `28/28`.
