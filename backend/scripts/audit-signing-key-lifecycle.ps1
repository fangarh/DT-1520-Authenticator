...
try {
    ...
    if ($ListRecent) {
        dotnet run --no-build --project $migrationsProjectPath -- list-signing-key-lifecycle-audit-events $Limit
    }
    else {
        dotnet run --no-build --project $migrationsProjectPath -- audit-signing-key-lifecycle
    }

    if ($LASTEXITCODE -ne 0) {
        throw "Signing key lifecycle audit command failed with exit code $LASTEXITCODE."
    }
}
finally {
    Pop-Location
